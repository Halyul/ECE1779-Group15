...
scaler_test_requests.set_scaler_config(0.8, min_threshold, 8, 0.5, True)
scaler_test_requests.set_scaler_cache_config(1, 'rr')
# scaler_test_requests.set_test_miss_rate(0.9) # force to increase pool size
RW_ratio = 0.5
key_list = []
failed_request = 0
total_get = 0
...
for i in range(60):
    ...
    min_threshold_list.append(min_threshold)
...
plt.xlabel('Time')
plt.savefig("Pool Size vs. Time - shrink.jpg")
plt.clf()

plt.plot(time_stamp, miss_rate, label = "miss rate")
plt.plot(time_stamp, min_threshold_list, label = "min threshold")
plt.legend()
plt.title("Miss Rate vs. Time")
plt.ylabel('Miss Rate')
plt.xlabel('Time')
plt.savefig("Miss Rate vs. Time - shrink.jpg")
plt.clf()
...
plt.plot(x_plt, [min_threshold] * len(x_plt), label = 'min threshold')
plt.legend()
plt.title("Miss Rate vs. Pool Size")
plt.ylabel('Miss Rate')
plt.xlabel('Pool Size')
plt.savefig("Miss Rate vs. Pool Size - shrink.jpg")
plt.clf()